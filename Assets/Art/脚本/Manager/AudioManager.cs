...
public class AudioManager : MonoSingleton<AudioManager>
{
    ...
    public void PlaySound(string path, AudioType _audioType, Vector3 _playPoint, float _minTone = 0.8f, float _maxTone = 1.2f, float _minDis = 10f, float _maxDis = 20f)
    {
        //PlayOneShot可以叠加播放
        switch (_audioType)
        {
            case AudioType.SFX:
                PlaySFX(GetAudio(path, _audioType), _playPoint,_minTone, _maxTone,_minDis,_maxDis);
                break;
            case AudioType.BGM:
                bgmSource.PlayOneShot(GetAudio(path, _audioType));
                break;
            case AudioType.Sound:
                soundSource.PlayOneShot(GetAudio(path, _audioType));
                break;
            default:
                Debug.LogError("错误类型");
                break;
